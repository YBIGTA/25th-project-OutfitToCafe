...
folder_path = '/content/drive/MyDrive/cafe_summary'
cafe_similarities = {}
...
for filename in os.listdir(folder_path):
    if filename.endswith('.json'):
        with open(os.path.join(folder_path, filename), 'r') as f:
            ...
            summary_text = cafe_data.get("summary", "")

            if summary_text:
                # 요약 텍스트의 임베딩 계산
                cafe_embedding = get_embedding(summary_text)

                # 스타일별 유사도 계산
                similarities = {}
                for style, style_embedding in style_embeddings.items():
                    similarity = cosine_similarity(cafe_embedding, style_embedding)[0][0]
                    similarities[style] = float(similarity)  # float32를 표준 float로 변환

                # 유사도 높은 순으로 정렬
                sorted_similarities = sorted(similarities.items(), key=lambda item: item[1], reverse=True)

                # 카페 이름과 함께 결과 저장
                cafe_name = filename.split('_')[1]  # 파일명에서 카페 이름 추출 (예: "summary_BNHR 성수_1223354536.json")
                cafe_similarities[cafe_name] = sorted_similarities

# 결과를 JSON 파일로 저장
output_file = '/content/drive/MyDrive/cafe_style_similarities.json'
