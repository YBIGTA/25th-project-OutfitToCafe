...
import tensorflow_text as text  # 이 라이브러리는 BERT 전처리기를 지원합니다.
import numpy as np
from sklearn.metrics.pairwise import cosine_similarity
import pandas as pd

# BERT 전처리기 및 인코더 모델 URL
...
# 텍스트 전처리 및 BERT 임베딩 생성
def get_bert_embedding(texts):
    inputs = preprocessor(texts)
    outputs = encoder(inputs)
    return outputs['pooled_output']  # [CLS] 토큰에 대한 출력 (임베딩)

# 텍스트 및 키워드에 대해 BERT 임베딩 생성
text_embedding = get_bert_embedding([text])
keyword_embeddings = get_bert_embedding(keywords)

# 코사인 유사도 계산
cosine_similarities = cosine_similarity(text_embedding, keyword_embeddings).flatten()
