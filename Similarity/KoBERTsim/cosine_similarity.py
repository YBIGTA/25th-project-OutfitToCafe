# from transformers import BertModel, BertTokenizer
# import torch
# from sklearn.metrics.pairwise import cosine_similarity
# import pandas as pd
import json
from functions import *


def get_style_keyword(user_style):
    with open('../../style_keyword.json', 'r', encoding='utf-8') as f:
        data = json.load(f)

    user_style_keyword = ' '.join(data[user_style])

    return user_style_keyword
# print(get_style_keyword('레트로'))
            

# 설명 데이터 가져오기
summary_path = '../../카페키워드추출/cafe_summary'
files = get_all_file_lists(summary_path)

# 유저 스타일 반환값 (CNN모델 들어갈 곳) + 해당 키워드 임베딩
# 레트로 스포티 로맨틱 캐주얼 미니멀 걸리시 클래식 시크 고프코어 스트릿 워크웨어 시티보이
user_styles =['레트로', '스포티', '로맨틱', '캐주얼', '미니멀', '걸리시', '클래식', '시크', '고프코어', '스트릿', '워크웨어', '시티보이']
for user_style in user_styles:
    user_style_keyword = get_style_keyword(user_style)
    keyword_embeddings = get_bert_embedding(user_style_keyword)


    # 반환 딕셔너리
    similarity_df = {}

    # 모든 카페 설명 파일에 대해 불러오기
    for file in files:

        # JSON 파일을 dict로 불러오기
        with open(f'{summary_path}/{file}', 'r', encoding='utf-8') as f:
            data = json.load(f)

        text = data['summary']

        # print(text)
        # print(user_style_keyword)

        # 텍스트와 키워드들에 대해 BERT 임베딩 생성
        text_embedding = get_bert_embedding(text)
        

        # 코사인 유사도 계산
        cosine_similarities = cosine_similarity(text_embedding, keyword_embeddings)[0][0]

        # 유사도 결과를 데이터프레임으로 저장
        similarity_df[data['cafe_name']] = f'{cosine_similarities:.4f}'

    print(similarity_df)

    # 유사도 높은 순으로 정렬
    sorted_cafe_similarities = sorted(similarity_df.items(), key=lambda item: item[1], reverse=True)


    # 결과를 JSON 파일로 저장
    output_file = f'./{user_style}_스타일_추천카페_KoBert.json'
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(sorted_cafe_similarities, f, ensure_ascii=False, indent=4)

    # 전체 카페 갯수 출력
    total_cafes = len(sorted_cafe_similarities)
    print(f"총 {total_cafes}개의 카페가 분석되었습니다.")
    print(f"결과가 {output_file} 파일로 저장되었습니다.")